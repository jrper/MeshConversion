...
int get_physical_tag(vtkUnstructuredGrid* ugrid, int n)
{
...
int get_elementary_tag(vtkUnstructuredGrid* ugrid, int n)
{
  ...
  if (cd->HasArray("ElementaryEntities")){
    ...
    int id;
    vtkIntArray* data = vtkIntArray::SafeDownCast(cd->GetArray("ElementaryEntities"));
    id = data->GetValue(n-1);
...
void vtkGmshWriter::WriteData()
{
  ...
  for (int i=0;i<4;i++){
    ...
    for(int j=0;j<typeData[1];j++) {
      int eleMetaData[3]={n,get_physical_tag(input,n),get_elementary_tag(input,n)};
      if (this->isBinary==1) {
	GMSHfile.write(reinterpret_cast<char*>(&eleMetaData),3*sizeof(int));
      } else {
	GMSHfile <<n<<" "<<typeData[0]<<" "<<typeData[2]<<" "<<" "<<get_physical_tag(input,n)<<" "<<get_elementary_tag(input,n);
      }
